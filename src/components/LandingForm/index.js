...
const LandingForm = ({ children, siteTitle, title, page }) => {
  ...
  const SignInInfo = () => {
    ...
    return (
      <span className="my-4 block text-gray-700">
        ...
        <Link to="/account/sign-up" className="hover:underline text-main-700">
          Register
        </Link>
      </span>
    )
  ...
  const SignUpInfo = () => {
    ...
    return (
      <span className="my-4 block text-gray-700">
        <Link to="/account/sign-in" className="hover:underline text-main-700">
  ...
  return (
    <div className="py-12 -mt-16 px-6 md:px-12 shadow bg-white rounded-lg w-full sm:w-2/3 md:w-1/2 lg:w-2/5 h-auto">
      ...
      <h1 className="text-4xl text-main-700 font-primary">{title}</h1>
      <SignInInfo />
      <SignUpInfo />
      {children}
    </div>
